...
class GRB:
    def __init__(self):
        ...
        t_min = np.log10(1e0)
        t_max = np.log10(1e5)
        self.time_array = np.logspace(t_min, t_max, 25)
        self.energy_array_min = [1e9 / AMES.eV2Hz, 1e11,  1,  1e3, 1e9, 1e11]
        self.energy_array_max = [1e9 / AMES.eV2Hz, 1e11, 1, 1e3, 1e9, 1e11]
    ...
    def plot_flux(self):
        ...
        ax.plot(data[:, 0], data[:, 4], 'C3-')
        ax.plot(data[:, 0], data[:, 5], 'C4-')
        ax.plot(data[:, 0], data[:, 6], 'C5-')
